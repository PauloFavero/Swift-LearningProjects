...
var stackOfOperators=Stack<Character>()
var stackOfValues=Stack<Int>()
...
for character in expression.characters {
    ...
    case "*":
        stackOfOperators.push(character)
    case "0"..."9":
        var value=Int(String(character))
        stackOfValues.push(value!)
    case ")":
    var char=stackOfOperators.pop()
        if char=="*"{
            result=stackOfValues.pop()!*stackOfValues.pop()!
            stackOfValues.push(result)
